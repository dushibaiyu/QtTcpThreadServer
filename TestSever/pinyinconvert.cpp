...
{
    outbuf = new char[HZ2PY_OUTPUT_BUF_ARRAY_SIZE];
    inbuf = new char[2048];
}
...
QString PinYinConvert::convertToPY(QString str, bool m_blnSimaple, bool polyphone_support,
                                   bool m_blnFirstBig, bool m_blnAllBiG, bool m_LetterEnd,
                                   bool m_unknowSkip, bool m_filterPunc)
{
    str.remove(' ');
    strcpy(inbuf,str.toStdString().c_str());
    memset(outbuf, '\0', sizeof(char) * HZ2PY_OUTPUT_BUF_ARRAY_SIZE);
    if(is_utf8_string(inbuf))
    {
        pinyin_utf8(inbuf,outbuf,m_blnSimaple,polyphone_support,m_blnFirstBig,
                    m_blnAllBiG,m_LetterEnd,m_unknowSkip,m_filterPunc);
    }
    else
    {
        pinyin_gb2312(inbuf,outbuf,m_blnSimaple,polyphone_support,m_blnFirstBig,
                      m_blnAllBiG,m_LetterEnd,m_unknowSkip,m_filterPunc);
    }
...
int PinYinConvert::is_utf8_string(char *utf)
{
...
void PinYinConvert::pinyin_utf8(char* inbuf,char* outbuf, bool m_blnSimaple ,bool polyphone_support ,bool m_blnFirstBig ,bool m_blnAllBiG ,bool m_LetterEnd , bool m_unknowSkip,bool m_filterPunc)
{
...
void PinYinConvert::pinyin_gb2312(char* inbuf,char* outbuf, bool m_blnSimaple ,bool polyphone_support  ,bool m_blnFirstBig ,bool m_blnAllBiG ,bool m_LetterEnd , bool m_unknowSkip,bool m_filterPunc)
{
    /*
    ...
    */
    ...
    for (int i=0; i<inbuf_len; i++)
    {
        ...
        if(!m_blnAllBiG)
        {
            for(j=1;j<iPinyinValue;j++)
            {
                if (pinyinValue[j]>='A'&&pinyinValue[j]<='Z')
                {
        ...
        if(!safeAddToOutbuf(outbuf,iOutbuf,pinyinValue,iPinyinValue)) return;
        i++;
        if(m_LetterEnd) if(!safeAddToOutbuf(outbuf,iOutbuf,&sep,1)) return;
    }
}

int PinYinConvert::findLetter(int nCode, char* strValue)
{
    switch(nCode)
    {
    case 6325:
    ...
    case 8043:
    case 8457:
        strcpy_s(strValue,HZ2PY_MAX_PINYIN_SIZE,"4337 伺");
        break;
    case 6042:
    case 6840:
    case 7085:
...
bool PinYinConvert::safeAddToOutbuf(char* outbuf,int &iOutbuf,char* pinyinValue,int iPinyinValue) {
...
const char * PinYinConvert::_pinyin_table_[20902] = {
